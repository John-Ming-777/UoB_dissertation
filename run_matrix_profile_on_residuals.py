...
# Input directory for the residuals data generated by Prophet
RESIDUALS_DIR = os.path.join(BASE_DIR, 'prophet_results', 'csv_results')
# Output directory for the Matrix Profile results
...
def analyze_residual_file(file_path):
    """Loads a residual file, runs Matrix Profile, and saves the output plot."""
    try:
        df = pd.read_csv(file_path)
        asset_name = os.path.basename(file_path).replace('_prophet_residuals.csv', '')
        print(f"--- Analyzing residuals for: {asset_name} ---")
    ...
    print(f"  Found {len(discords)} discords. Plotting and saving top {top_n_discords}...")
...
if __name__ == '__main__':
    print("Starting Matrix Profile analysis on all Prophet residual files...")
    # Find all residual files
    residual_files = glob.glob(os.path.join(RESIDUALS_DIR, '*_prophet_residuals.csv'))
    
    if not residual_files:
        print("No residual files found. Please run the Prophet analysis first.")
    else:
        for f in residual_files:
            analyze_residual_file(f)
        print("\nMatrix Profile analysis completed for all available residual files.")
